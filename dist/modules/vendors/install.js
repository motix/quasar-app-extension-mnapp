...
`;
        ...
        if (!dotenvlocal.includes('FONTAWESOME_PACKAGE_TOKEN')) {
            fs.writeFileSync(api.resolve.app('.env.local-mnapp-fap'), `${dotenvlocal === '' ? '' : dotenvlocal.trim() + '\n\n'}FONTAWESOME_PACKAGE_TOKEN=XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX
`);
            console.log(' \x1b[32mvendors       • \x1b[0m\x1b[47m\x1b[30m./.env.local-mnapp-fap\x1b[0m with Font Awesome Pro token \x1b[33mFONTAWESOME_PACKAGE_TOKEN\x1b[0m is missing. The extension could be installed successfully but the process will be terminated at dependencies installation step.');
        }
    }
});
